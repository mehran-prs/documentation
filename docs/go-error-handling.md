...
If the Activity returns an error as `NewNonRetryableApplicationError(message, errType, cause, details)`, that error will be returned as `*temporal.ApplicationError`.
